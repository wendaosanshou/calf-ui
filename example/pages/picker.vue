<template>
  <div class="page-picker">
    ...
    <calf-button @click="handlePicker5">城市</calf-button>
    <calf-button @click="handlePicker6">较少选项</calf-button>
    <calf-button @click="pickerVisible = true">测试</calf-button>
...
export default {
  ...
  methods: {
    ...
    selectDateHandle(selectedVal, selectedIndex, selectedText) {
      this.$createDialog({
        content: `Selected Item: <br/>  value: ${selectedVal} <br/>  index: ${selectedIndex.join(', ')} <br/>  text: ${selectedText.join(' ')}`
      }).show();
    },
    cancelHandle() {}
  },

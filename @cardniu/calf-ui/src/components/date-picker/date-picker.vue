...
const COMPONENT_NAME = 'calf-date-picker'
  ...
  computed: {
    ...
    natureRangeCache() {
      ...
      Object.keys(natureRangeCache).forEach(key => {
        natureRangeCache[key] = this._range(
          key,
          NATURE_BOUNDARY_MAP[key].natureMin,
          NATURE_BOUNDARY_MAP[key].natureMax
        )
      })
